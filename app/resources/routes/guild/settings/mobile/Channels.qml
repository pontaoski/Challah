...
Kirigami.ScrollablePage {
    ...
    ColumnLayout {
		QQC2.RoundButton {
			...
			onClicked: textAsker.ask(qsTr("What do you want to call the new voice channel?")).then((name) => delModel.model.newChannel(name, "voice"))
		...
		QQC2.RoundButton {
			...
			enabled: tryit(() => canCreateChannels.data.has, false)
			onClicked: textAsker.ask(qsTr("What do you want to call the new channel?")).then((name) => delModel.model.newChannel(name))
            Layout.alignment: Qt.AlignRight
		}
